...
The Windows installer downloads `solder-<version>-windows-x64.zip`, installs to `$env:LOCALAPPDATA\Solder`, adds the installed `bin` directory to the user `PATH` if needed, sets user `KICAD_APP_PATH` and `KICAD_CLI_PATH` variables for the bundled SolderCAD tools, and clears any previous `KICAD_SOFTWARE_RENDERING` override.
